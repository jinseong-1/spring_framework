...
public class BoardServiceImpl implements BoardService {
	...
	public String regist(BoardVO board) throws Exception {
		...
		String message = (result != 0) ? "SECCESS" : "FAILED";
		return message;
	}
	...
	public List<BoardVO> listCriteria(Criteria cri) throws Exception {
		// database 에서 criteria 정보를 이용하여 사용자가 요청한 페이지에 따라 게시글 목록을 검색하여 전달
		return dao.listCriteria(cri);
	}

	@Override
	public PageMaker getPageMaker(Criteria cri) throws Exception {
		// criteria 요청한 페이지 정보에 따라 페이징 블럭 정보를 저장하는 PageMaker 객체 생성 및 반환
