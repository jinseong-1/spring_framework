...
public class BoardController {
	...
	public String readPage(
			...
			@ModelAttribute("cri") Criteria cri, int bno, Model model, HttpSession session) throws Exception {
		/*
		// readPage로 session 에 등록된 result message 가 존재하는지 확인
	...
	public String modifyPost(//HttpSession session,
			RedirectAttributes rttrs,
			BoardVO board, Model model) throws Exception{
		String result = bs.modify(board);
		// session.setAttribute("result",result);
		rttrs.addFlashAttribute("result",result);	// 세션을 model에 전달하고 session을 삭제하는걸 담당
		rttrs.addAttribute("bno",board.getBno());	// 메세지를 남겨주는 것
